...
def insert_data_into_db(data):
    ...
    for session in data:
        ...
        cursor.execute('''
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        ''', (session['activity_id'], session['timestamp'], session['sport'], session['avg_heart_rate'], session['total_elapsed_time'], session['distance'], session['fat'], session['Total Fat'], session['Carbs'], session['Total Carbs'], session['VO2maxSmooth'], session['VO2maxSession'], session['CardiacDrift'], session['CooperTest'], session['Steps'], session['field110'], session['Running Economy']))
